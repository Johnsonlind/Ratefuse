...
import { deleteResource, fetchMyFavoriteResources, fetchMySharedResources, type ResourceItem, type ResourceType, RESOURCE_TYPES, updateResource } from '../api/resources';
import { ResourcePlatformRow } from '../modules/resources/ResourcePlatformRow';
import { ResourceLinkDialog } from '../modules/resources/ResourceLinkDialog';
import { MEMBERSHIP_ENABLED } from '../config/features';
import { hasMemberPrivileges } from '../utils/membershipAccess';
...
export default function ProfilePage() {
  ...
  const [activeTab, setActiveTab] = useState<'collections' | 'following' | 'feedbacks' | 'resources'>('collections');
  ...
  const [resourceDialogType, setResourceDialogType] = useState<ResourceType>('baidu');
  const [resourceDialogExisting, setResourceDialogExisting] = useState<ResourceItem | null>(null);
  const [buying, setBuying] = useState(false);
  const activeFeedbackChatRef = useRef<HTMLDivElement | null>(null);

  const sharedQuery = useQuery({
    queryKey: ['profile', 'resources', 'shared'],
    queryFn: fetchMySharedResources,
    enabled: hasMemberPrivileges(user) && activeTab === 'resources',
    refetchOnWindowFocus: true,
    staleTime: 0,
  });

  const favQuery = useQuery({
    queryKey: ['profile', 'resources', 'favorites'],
    queryFn: fetchMyFavoriteResources,
    enabled: hasMemberPrivileges(user) && activeTab === 'resources',
    refetchOnWindowFocus: true,
    staleTime: 0,
  });
  ...
  const sharedGroups = useMemo(() => {
    const rows = (sharedQuery.data || []) as any[];
    ...
    return Array.from(byKey.entries()).map(([key, items]) => {
      const [media_type, tmdbIdStr] = key.split(':');
      const tmdb_id = Number(tmdbIdStr);
      const title = items[0]?.media_title || `TMDB:${tmdb_id}`;
      const year = items[0]?.media_year;
      const resourcesByType: Partial<Record<ResourceType, ResourceItem>> = {};
      const canEditByType: Partial<Record<ResourceType, boolean>> = {};
      for (const item of items) {
        resourcesByType[item.resource_type] = item;
        canEditByType[item.resource_type] = !!(item as any).can_edit;
      }
      return { media_type, tmdb_id, title, year, resourcesByType, canEditByType };
    });
  }, [sharedQuery.data]);

  const favGroups = useMemo(() => {
    const rows = (favQuery.data || []) as any[];
    const byKey = new Map<string, ResourceItem[]>();
    for (const r of rows) {
      const key = `${r.media_type}:${r.tmdb_id}`;
      const arr = byKey.get(key) || [];
    ...
    return Array.from(byKey.entries()).map(([key, items]) => {
      ...
      const year = items[0]?.media_year;
      const resourcesByType: Partial<Record<ResourceType, ResourceItem>> = {};
      for (const item of items) resourcesByType[item.resource_type] = item;
      return { media_type, tmdb_id, title, year, resourcesByType };
    });
  }, [favQuery.data]);
    ...
    <PageShell maxWidth="4xl" contentClassName="py-3">
                    ...
                    <>
                      ...
                      {MEMBERSHIP_ENABLED && (
                        <div className="mt-2 text-sm text-gray-600 dark:text-gray-300">
                          {user?.is_member ? `会员有效期至：${formatChinaDateTime(user.member_expired_at || '')}` : '未开通会员'}
                        </div>
                      )}
                        ...
                        {MEMBERSHIP_ENABLED && (
                          <button
                            ...
                            onClick={async () => {
                              ...
                              setBuying(true);
                              try {
                                await authFetchJson('/api/member/orders', {
                                  method: 'POST',
          ...
          <div className="glass-card rounded-2xl p-8">
            <CardTabs
              tabs={[
                ...
                ...(hasMemberPrivileges(user) ? [{ id: 'resources', label: '我的资源' }] : []),
                { id: 'feedbacks', label: '我的反馈' },
              ]}
              activeId={activeTab}
              onChange={(id) => setActiveTab(id as 'collections' | 'following' | 'feedbacks' | 'resources')}
            />
            ...
            {activeTab === 'resources' && hasMemberPrivileges(user) && (
              <div className="mt-6 grid gap-4 md:grid-cols-2">
                <div className="glass-card rounded-xl p-4">
                  <div className="mb-3 flex items-center justify-between">
                    <div className="text-sm font-semibold">分享</div>
                    <button
                      type="button"
                  ...
                  {sharedQuery.isLoading ? (
                    ...
                    <div className="space-y-3">
                      {sharedGroups.map((g) => (
                        <div key={`${g.media_type}:${g.tmdb_id}`} className="rounded-xl border border-white/10 bg-white/20 dark:bg-gray-900/20 p-3">
                          <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                              ...
                              <div className="mt-0.5 text-xs text-gray-500">
                                {g.media_type === 'movie' ? '电影' : '剧集'} · TMDB {g.tmdb_id}
                              </div>
                            </div>
                            <button
                              type="button"
                              className="shrink-0 text-xs text-gray-500 hover:text-blue-500"
                              onClick={() => navigate(`/${g.media_type === 'movie' ? 'movie' : 'tv'}/${g.tmdb_id}`)}
                            >
                              查看
                            </button>
                          </div>
                          <div className="mt-3">
                            <ResourcePlatformRow
                              ...
                              resourcesByType={g.resourcesByType}
                              canEditByType={g.canEditByType}
                              onAddOrEdit={(t, existing) => {
                                // Profile页仅允许编辑“自己分享的资源”，无资源不显示添加入口（避免重复入口）
                                if (!existing) return;
                                setResourceDialogType(t);
                                setResourceDialogExisting(existing);
                                setResourceDialogOpen(true);
                              }}
                              onOpen={(resource) => {
                                // 分享页点击：如果不是可编辑（理论上都是可编辑），也按打开处理
                                window.open(resource.link, '_blank', 'noreferrer');
                              }}
                              onDelete={async (resource) => {
                                await deleteResource(resource.id);
                                queryClient.invalidateQueries({ queryKey: ['profile', 'resources', 'shared'] });
                                queryClient.invalidateQueries({ queryKey: ['profile', 'resources', 'favorites'] });
                                toast.success('已删除该平台资源');
                              }}
                            />
                          </div>
                        </div>
                      ))}
                      {sharedGroups.length === 0 && <div className="text-sm text-gray-500">暂无分享资源</div>}
                    </div>
                  )}
                </div>
                <div className="glass-card rounded-xl p-4">
                  <div className="mb-3 flex items-center justify-between">
                    <div className="text-sm font-semibold">收藏</div>
                    <button
                      type="button"
                      className="text-xs text-blue-500"
                    ...
                    <div className="space-y-3">
                      {favGroups.map((g) => (
                        <div key={`${g.media_type}:${g.tmdb_id}`} className="rounded-xl border border-white/10 bg-white/20 dark:bg-gray-900/20 p-3">
                          ...
                          <div className="mt-3">
                            <ResourcePlatformRow
                              types={Object.keys(g.resourcesByType) as ResourceType[]}
                              resourcesByType={g.resourcesByType}
                              favoriteConfirmUnfavorite
                              onFavoriteChange={() => {
                                queryClient.invalidateQueries({ queryKey: ['profile', 'resources', 'favorites'] });
                                queryClient.invalidateQueries({ queryKey: ['profile', 'resources', 'shared'] });
                              }}
                      ...
                      {favGroups.length === 0 && <div className="text-sm text-gray-500">暂无收藏资源</div>}
                    </div>
                  )}
                </div>
              </div>
      ...
      >
        ...
        <ResourceLinkDialog
          ...
          resourceType={resourceDialogType}
          existing={resourceDialogExisting}
          onSubmit={async ({ link, extraction_code }) => {
            if (!resourceDialogExisting?.id) return;
            await updateResource(resourceDialogExisting.id, { link, extraction_code });
            queryClient.invalidateQueries({ queryKey: ['profile', 'resources', 'shared'] });
            queryClient.invalidateQueries({ queryKey: ['profile', 'resources', 'favorites'] });
          }}
        />
